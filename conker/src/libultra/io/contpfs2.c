...
/* Duplicate of contpfs: *2 entry points call the init (non-2) helpers. */

u16 __osSumcalc2(u8 *ptr, int length)
{
    int i;
    u32 sum;
    u8 *tmp;
...
s32 __osIdCheckSum2(u16 *ptr, u16 *csum, u16 *icsum)
{
    u16 data;
    u32 j;
    data = 0;
...
s32 __osRepairPackId2(OSPfs *pfs, __OSPackId *badid, __OSPackId *newid)
{
    s32 ret;
    u8 temp[32];
    u8 comp[32];
...
s32 __osCheckPackId2(OSPfs *pfs, __OSPackId *temp)
{
    u16 index[4];
    s32 ret;
    u16 sum;
...
s32 __osPfsRWInode2(OSPfs *pfs, __OSInode *inode, u8 flag, u8 bank)
{
    u8 sum;
    int j;
    s32 ret;
...
s32 __osPfsSelectBank2(OSPfs *pfs)
{
    u8 temp[BLOCKSIZE];
    int i;
    s32 ret;
