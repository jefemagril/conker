...
/* Twin of Perfect Dark mp3_make_samples, with Conker stream teardown / LP load path. */
s32 n_alStreamMakeSamples(s32 samples, Acmd **cmd) {
    ...
    if (gStreamState == STREAM_STATE_PLAYING) {
        if (samples + D_800E0DF0 > D_800E0DEC) {
            ...
            if (D_800E0DEC != 0 && D_800E0DE8 != NULL) {
                sp54 = (u16 *) ((D_800E0DF0 * 2) + (s8 *) D_800E0DE8);
                sp50 = D_800E0DEC;
            }
        ...
        if (D_800E0E19 == 0) {
            ...
            if (gStreamLoadCommandCount > 0) {
                ...
                if (gStreamLoadMode != 0) {
                    n_alFxInitlpfilter_mono((ALLowPass *) ((s8 *) &D_800E0D80 + 0x30), 22020.0f);
                }
                loadCmd = (*cmd)++;
                ...
                {
                    Acmd *loadCmd2 = (*cmd)++;
                    loadCmd2->words.w0 = 0x0E000000 | ((gStreamLoadMode & 0xFF) << 16);
                    loadCmd2->words.w1 = osVirtualToPhysical(D_800E0DDC) & 0xFFFFFF & 0xFFFFFF;
                }
                gStreamLoadMode = 0;
            }
    ...
    } else if ((gStreamState == STREAM_STATE_TEARDOWN) && (D_800BE9F0 != 0x35)) {
        if (gStreamInitialized != 0) {
            func_10004074(gStreamVoice0);
            func_10004074(gStreamVoice1);
            func_10004074(gStreamVoice2);
