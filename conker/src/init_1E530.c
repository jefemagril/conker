...
/* 1.4141999483109f; lives in its own rodata split ahead of this file's */
extern f32 D_8002C7A0;

Acmd *n_alFxPull(s32 sampleOffset, Acmd *p, s32 fxBus)
{
    Acmd *ptr = p;
    /* Conker keeps the bus FX pointer in the ALAuxBus `sources` slot (0x1C) */
    ALFx *r = (ALFx *) n_syn->auxBus[fxBus].sources;
     ...
     */
    ptr = n_alAuxBusPull(sampleOffset, p, fxBus, &numpulls);
    ...
    if (gAudioOutputLeftMixFlags[fxBus] == 0) {
    ...
    if (gAudioOutputLeftMixFlags[fxBus]) {
        ptr = _n_saveBuffer(r, 1, r->input[1], N_AL_AUX_R_OUT, ptr);
    }

    for (j = 0; j <= gAudioOutputLeftMixFlags[fxBus]; j++) {
        ...
        for (i = 0; i < r->section_count; i++) {
            ...
            if (gAudioOutputRightMixFlags[fxBus] && gAudioOutputLeftMixFlags[fxBus]) {
                d->ffcoef = -d->ffcoef;
                d->fbcoef = -d->fbcoef;
            }
            ...
            if (d->gain) {
                if (gAudioOutputLeftMixFlags[fxBus]) {
                    ...
                    u32 gain = d->gain * D_8002C7A0;

                    if (gain > 0x7fff) {
                        gain = 0x7fff;
                    }

                    aMix(ptr++, 0, (u16) gain, buff2, output);
                }
            }
        ...
        if (gAudioOutputLeftMixFlags[fxBus] && j == 0) {
            ptr = _n_loadBuffer(r, 1, r->input[1], input, FIXED_SAMPLE, ptr);

            if (gAudioOutputRightMixFlags[fxBus]) {
                aMix(ptr++, 0, 0x5a82, output, N_AL_MAIN_R_OUT);
            } else {
                aMix(ptr++, 0, 0x5a82, output, N_AL_MAIN_L_OUT);
            }
        } else {
            /* empty */
        }
...
/*
 * This routine gets called by n_alSynSetFXParam. No checking takes place to
...
#define LENGTH  (f->delay[s].output - f->delay[s].input)
    ...
    if (f->delay[s].rs) {
        if (p != CHORUSDEPTH_PARAM) {
            if (LENGTH != 0) {
                rsgain = f->delay[s].rsgain / (f->delay[s].output - f->delay[s].input) * CONVERT;
            } else {
                rsgain = 0;
            }
        }
